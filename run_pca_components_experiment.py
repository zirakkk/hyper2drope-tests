#!/usr/bin/env python3
"""
PCA Components Sensitivity Analysis for KSC Dataset
...
def update_pca_components(config_path, pca_components):
    """Update PCA components for KSC dataset in config file"""
    print(f"Updating PCA components to {pca_components} in {config_path}")
    
    config_data = load_config(config_path)
    config_data['datasets']['KSC']['pca'] = pca_components
    save_config(config_path, config_data)
    
    print(f"Updated KSC pca to: {pca_components}")
...
def run_experiment(pca_components):
    ...
    current_pca = config_data['datasets']['KSC']['pca']
...
def main():
    ...
    print("Starting PCA components sensitivity analysis for KSC dataset...")
